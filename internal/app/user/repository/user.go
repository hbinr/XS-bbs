...
// Insert 新增用户
func (u *userRepo) Insert(user *model.User) (err error) {
	return u.db.Create(&user).Error
}

// Delete 根据用户ID删除用户，软删除
func (u *userRepo) Delete(userID int64) bool {
...
func (u *userRepo) GetUserByName(userName string) (user *model.User, err error) {
	user = new(model.User)
...
func (u *userRepo) GetUserByID(userID int64) (user *model.User, err error) {
	user = new(model.User)
...
func (u *userRepo) CheckUserByUserName(userName string) error {
	var count int64
...
func (u *userRepo) CheckUserByEmail(email string) error {
	var count int64
